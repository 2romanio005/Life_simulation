...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)  // обработка событий
{
	switch (message)
	{
		...
		//case WM_VSCROLL:   // вертикальные
		{
			...
			int max_pos = -1;
			int one_step = 1;
			if (SliderTimeDraw == (HWND)lParam) { type = 0; pos = &PosSliderTimeDraw; min_pos = 1; max_pos = MaxPosSliderTimeDraw; one_step = (MaxPosSliderTimeDraw + 4) / 5; }
			else
				if (SliderStepDraw == (HWND)lParam) { type = 1;	pos = &PosSliderStepDraw; min_pos = 1; max_pos = MaxPosSliderTimeDraw; one_step = (MaxPosSliderTimeDraw + 4) / 5; }
				else
					if (SliderPeepBrain == (HWND)lParam) { type = 2; pos = &PosSliderPeepBrain; min_pos = 0; max_pos = MaxPosSliderPeepBrain; one_step = (MaxPosSliderPeepBrain + 9) / 10; }
					else

						if (SliderRedEat == (HWND)lParam) { type = 3; pos = &PosSliderRedEat; min_pos = 0; max_pos = MaxPosSliderRedEat; one_step = 10; }
						else
							if (SliderRedLeave == (HWND)lParam) { type = 4; pos = &PosSliderRedLeave; min_pos = 0; max_pos = MaxPosSliderRedLeave; one_step = (MaxPosSliderRedLeave + 9) / 10; }
							else
								if (SliderGreenEat == (HWND)lParam) { type = 5; pos = &PosSliderGreenEat; min_pos = 0; max_pos = MaxPosSliderGreenEat; one_step = 10; }
								else
									if (SliderBlueEat == (HWND)lParam) { type = 6; pos = &PosSliderBlueEat; min_pos = 0; max_pos = MaxPosSliderBlueEat; one_step = 10; }
									else
